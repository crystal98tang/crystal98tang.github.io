...
```shell
tang@SC-201708020022:~$ cat < testfile
...
```shell
tang@SC-201708020022:~$ ls -l > test2
tang@SC-201708020022:~$ cat test2
总用量 28
drwxrwxrwx 0 tang tang 4096 8月   2 11:48 biosoft
-rw-rw-rw- 1 tang tang 2156 8月   4 00:12 biostar.yml
drwxrwxrwx 0 tang tang 4096 8月   3 22:24 miniconda2
drwxrwxrwx 0 tang tang 4096 8月   2 11:50 ncbi
-rw-rw-rw- 1 tang tang 5230 8月  14 00:14 spf13-vim.sh
drwxrwxrwx 0 tang tang 4096 8月  13 23:51 src
-rw-rw-rw- 1 tang tang    0 8月  21 22:43 test2
-rw-rw-rw- 1 tang tang   73 8月  21 22:39 testfile
drwxrwxrwx 0 tang tang 4096 8月  19 17:15 tmp
-rw-rw-rw- 1 tang tang 2156 8月  14 12:20 wsx_biostar.yml
```
...
1. 只重定向错误
   ...
   ```shell
   tang@SC-201708020022:~$ ls -al badfile 2> test4
   tang@SC-201708020022:~$ cat test4
...
2. 重定向错误和数据
   ...
   ```shell
   tang@SC-201708020022:~$ ls -al test test2 test3 bad test 2> test6 1> test7
   tang@SC-201708020022:~$ cat test6
   ls: 无法访问'test': 没有那个文件或目录
   ls: 无法访问'test3': 没有那个文件或目录
   ls: 无法访问'bad': 没有那个文件或目录
   ls: 无法访问'test': 没有那个文件或目录
   tang@SC-201708020022:~$ cat test7
   -rw-rw-rw- 1 tang tang 571 8月  21 22:43 test2
   ```

   可以看到正常输出重定向到了test7，错误重定向到了test6。另外，也可以将STDERR和STDOUT的输出重定向到同一个输出文件，bash shell提供了符号`&>`。

   ```shell
   tang@SC-201708020022:~$ ls -al test test2 test3 bad &> test7
   tang@SC-201708020022:~$ cat test7
   ls: 无法访问'test': 没有那个文件或目录
   ls: 无法访问'test3': 没有那个文件或目录
   ls: 无法访问'bad': 没有那个文件或目录
...
```shell
tang@SC-201708020022:~$ cat test8
#!/bin/bash
...
```shell
tang@SC-201708020022:~$ sh test8 2> test9
This is normal output
tang@SC-201708020022:~$ cat test9
This is an error
```
...
```shell
tang@SC-201708020022:~$ cat test10
#!/bin/bash
...
tang@SC-201708020022:~$ sh test10
tang@SC-201708020022:~$ cat testout
...
```shell
tang@SC-201708020022:~$ cat test11
#!/bin/bash
...
tang@SC-201708020022:~$ sh test11
This is the start of the script
now redirecting all output to another location
tang@SC-201708020022:~$ cat testout
This output should go to the testout file
tang@SC-201708020022:~$ cat testerror
...
```shell
tang@SC-201708020022:~$ cat test14
#!/bin/bash
...
tang@SC-201708020022:~$ sh test14
Now things should be back to normal
tang@SC-201708020022:~$ cat test14out
...
```shell
tang@SC-201708020022:~$ ls -al > /dev/null
tang@SC-201708020022:~$ cat /dev/null
tang@SC-201708020022:~$
```

这是避免出现错误消息，也无需保存它们的一个常用方法。

```shell
tang@SC-201708020022:~$ ls -al badfile test2 2> /dev/null
-rw-rw-rw- 1 tang tang 571 8月  21 22:43 test2
```
...
This is the second line.
This is the third line.
tang@SC-201708020022:~$ cat /dev/null > testfile
tang@SC-201708020022:~$ cat testfile
tang@SC-201708020022:~$
```
...
```shell
tang@SC-201708020022:~/tmp$ mktemp testing.XXXXXX
tang@SC-201708020022:~/tmp$ mktemp testing.XXXXXX
tang@SC-201708020022:~/tmp$ ll
总用量 12
drwxrwxrwx 0 tang tang 4096 8月  22 21:34 ./
drwxr-xr-x 0 tang tang 4096 8月  22 21:31 ../
-rw------- 1 tang tang    0 8月  22 21:33 testing.R6dAku
-rw------- 1 tang tang    0 8月  22 21:32 testing.V5psXP
```
...
```shell
tang@SC-201708020022:~/tmp$ cat test19
#!/bin/bash t nomore" \
...
tang@SC-201708020022:~/tmp$ sh test19
...
```shell
tang@SC-201708020022:~/tmp$ mktemp -t test20.XXXX
/tmp/test20.bY3Q
tang@SC-201708020022:~/tmp$ mktemp -t test20.XXXXXX
/tmp/test20.WrkAia
```
...
```shell
tang@SC-201708020022:~/tmp$ date | tee testfile
2017年 08月 22日 星期二 21:49:07 DST
tang@SC-201708020022:~/tmp$ cat testfile
2017年 08月 22日 星期二 21:49:07 DST
```
...
```shell
tang@SC-201708020022:~/tmp$ cat members.csv
...
```shell
tang@SC-201708020022:~/tmp$ ./test23 members.csv
tang@SC-201708020022:~/tmp$ cat members.sql
    INSERT INTO members (lname,fname,address,city,state,zip) VALUES
    ('Blum', 'Richard', '123 Main St.', 'Chicago', 'IL', '60601');
    INSERT INTO members (lname,fname,address,city,state,zip) VALUES
    ('Blum', 'Barbara', '123 Main St.', 'Chicago', 'IL', '60601');
